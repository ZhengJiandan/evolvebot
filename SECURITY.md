...
If you discover a security vulnerability in evolvebot, please report it by:
...
chmod 600 ~/.evolvebot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.evolvebot/config.json` with file permissions set to `0600`
...
- ✅ Run evolvebot with a dedicated user account
...
- Keep authentication data in `~/.evolvebot/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade evolvebot-ai
```
...
- Subscribe to security advisories for evolvebot and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash evolvebot
   sudo -u evolvebot evolvebot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.evolvebot
   chmod 600 ~/.evolvebot/config.json
   chmod 700 ~/.evolvebot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.evolvebot/logs/evolvebot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.evolvebot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.evolvebot/logs/evolvebot.log
   ```
